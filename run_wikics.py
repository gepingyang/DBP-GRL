...
def main():
    ...
    adj, feat, labels, num_class, _, _, _ = load('wikics')
    X = feat.to(device)
    F.normalize(X, out=X)
    A = adj.to(device)
    del adj, feat
    ...
    model = DBP_GRL(
        alpha_S=5e-2, alpha_A=1e-3,
        beta_S=1e-4, beta_A=1e-3,
        tau_S=0.99, tau_A=0.999,
        epoch=100, block_num=1,
    )
    print('Wikics | DBP_GRL | epoch=100 | block_num=1')
    ...
    for seed in range(10):
        ...
        train_idx, val_idx, test_idx = load_index('wikics')
