...
def main():
    ...
    adj, feat, labels, num_class, _, _, _ = load('co_physics')
    X = feat.to(device)
    F.normalize(X, out=X)
    A = adj.to(device)
    del adj, feat
    ...
    model = DBP_GRL(
        alpha_S=1e-1, alpha_A=1e-4,
        beta_S=1e-3, beta_A=1e-3,
        tau_S=0.95, tau_A=0.999,
        epoch=100, block_num=1,
    )
    print('CO_PHYSICS | DBP_GRL | epoch=100 | block_num=1')
    ...
    for seed in range(10):
        ...
        train_idx, val_idx, test_idx = load_index('co_physics')
